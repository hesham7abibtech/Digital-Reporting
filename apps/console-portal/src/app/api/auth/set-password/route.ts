...
/**
 * Set an admin's password from a verified Supabase session.
 * The caller proves ownership of the account via the access-token
 * (Authorization: Bearer); the server then sets the password + clears the
 * migration flag with the service_role. Used by the console's post-migration
