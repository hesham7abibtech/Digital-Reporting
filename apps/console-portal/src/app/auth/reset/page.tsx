...
function ResetPasswordContent() {
  ...
  // Wait for the Supabase session the admin established by signing in with the
  // one-time password (same origin → session lives in this tab's storage).
  useEffect(() => {
    let settled = false;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) throw new Error('Recovery session expired. Please request a new one-time password.');
      ...
      // Replace the one-time session with a clean full session under the new
      // credentials — the OTP session JWT still carries the stale
      // `must_set_password` claim, so without this the admin can be re-prompted.
      // Re-arm the console session marker so the dashboard guard lets them in.
      const { error: signInErr } = await supabaseBrowser.auth.signInWithPassword({ email, password: newPassword });
      if (!signInErr && typeof window !== 'undefined') {
        sessionStorage.removeItem('dashboard_session');
        sessionStorage.setItem('admin_session', 'active');
      }
      setStatus('success');
      setTimeout(() => router.push(signInErr ? '/admin/login' : '/admin/dashboard'), 3200);
    } catch (err: any) {
      setStatus('error');
    ...
    return (
      <div style={containerStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', padding: '16px 28px', borderRadius: 18, boxShadow: '0 10px 30px rgba(0,42,48,0.06)' }}>
          ...
          <p style={{ color: TEAL, fontWeight: 700, fontSize: 14 }}>Verifying your secure session…</p>
        </div>
      </div>
    );
  }
        ...
        <AnimatePresence mode="wait">
          {status === 'success' ? (
            ...
            <motion.div key="error" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 800, color: '#dc2626', marginBottom: 12 }}>Session Invalid</h2>
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: 'center', padding: '12px 0' }}>
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
        ...
        Your account is secured on the new platform.<br />Taking you to the Command Center…
      </motion.p>
    </motion.div>
  );
}
